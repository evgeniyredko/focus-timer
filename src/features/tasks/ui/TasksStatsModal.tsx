        ...
        {tasksFiltered.length === 0 ? (
          ...
          tasksFiltered.map(task => (
            <div key={task.id} className="flex justify-between pt-1 pb-1.5 text-2xl border-b">
              <span className="text-left w-full border-r pr-2 pl-1 font-semibold">{task.title}</span>
              <span className="pl-2 pr-1 font-semibold">{formatHHMMSS(task.todaySecondsSpent)}</span>
            </div>
          ))
        )}
        <button
