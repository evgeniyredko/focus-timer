...
import { requestNotificationsPermission, showAppNotification } from "../../../shared/lib/notifications";
...
export const completePhaseThunk =
  () => (dispatch: AppDispatch, getState: () => RootState) => {
    const state = getState();
    const { phase, cyclesCompleted } = state.timer;
    const { cyclesBeforeLongBreak, focusMinutes, shortBreakMinutes, longBreakMinutes } = state.settings;
    let nextPhase: Phase;
...
export const resetSessionThunk =
  () => (dispatch: AppDispatch, getState: () => RootState) => {
